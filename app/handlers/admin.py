...
async def _safe_edit(message, text, reply_markup=None):
    """Edit text safely; fallback to answer if 'message is not modified'."""
    try:
...
def _val(row, key, default=None):
    """Safe getter برای sqlite3.Row یا dict."""
    try:
        return row[key]
    except Exception:
        return default
...
async def _send_with_effect(bot, chat_id: int, text: str, effect_key: str):
    """Send message with message_effect_id if configured; fallback to normal."""
...
async def admin_effects(cb: CallbackQuery):
    ...
    txt = f"""✨ افکت‌ها (message_effect_id):
• ثبت سفارش ❤️: {created}
• اتمام 🎉: {completed}
• رد 👎: {rejected}

برای تغییر: دکمه را بزنید و یک «پیام با افکت» ارسال کنید."""
...
async def admin_set_effect_start(cb: CallbackQuery, state: FSMContext):
    ...
    await cb.message.answer("یک پیام با افکت بفرستید (روی Send نگه دارید و 🎉/❤️/👎 انتخاب کنید).")

@router.message(EffectStates.waiting)
async def admin_set_effect_save(m: Message, state: FSMContext):
    data = await state.get_data()
    k = data.get("effect_key")
    eff_id = getattr(m, "effect_id", None)
    if not eff_id:
        await m.answer("❌ این پیام افکت ندارد. دوباره تلاش کنید.")
        return
    await _set_setting(k, str(eff_id))
    await state.clear()
    await m.answer("✅ افکت ذخیره شد.", reply_markup=admin_effects_kb())
...
@router.callback_query(F.data.startswith("admin:prod_desc:"))
async def admin_prod_desc(cb: CallbackQuery, state: FSMContext):
    if not await guard_admin(cb):
        return
    pid = int(cb.data.split(":")[2])
    await state.update_data(prod_id=pid)
    await state.set_state(ProdStates.editing_desc)
    await cb.message.answer("📝 متن توضیح برای این محصول را ارسال کنید:")

@router.message(ProdStates.editing_desc, F.text)
async def admin_save_prod_desc(m: Message, state: FSMContext):
    desc = (m.text or "").strip()
    data = await state.get_data()
    pid = int(data.get("prod_id"))
    await _set_setting(f"PROD_DESC_{pid}", desc)
    await state.clear()
    prods = await fetchall("SELECT id, title FROM products ORDER BY id DESC")
    await m.answer("✅ توضیح محصول ذخیره شد.", reply_markup=admin_prods_kb(prods))
...
async def admin_add_plan_price(m: Message, state: FSMContext):
    ...
    await execute("INSERT INTO plans(product_id, title, price) VALUES(?,?,?)", pid, title, int(price_text))
    await state.clear()
    plans = await fetchall("SELECT id, title, price FROM plans WHERE product_id=? ORDER BY price ASC", pid)
...
async def plan_edit_price(m: Message, state: FSMContext):
    ...
    pid = int(_val(row, "product_id", 0))
    plans = await fetchall("SELECT id, title, price FROM plans WHERE product_id=? ORDER BY price ASC", pid)
    await m.answer("✅ قیمت پلن به‌روزرسانی شد.", reply_markup=admin_plans_list_kb(plans, pid))
...
async def admin_order_view(cb: CallbackQuery):
    ...
    row = await fetchone(
        """SELECT o.id, o.status, o.tracking_code, o.created_at,
...
async def admin_order_processing(cb: CallbackQuery):
    ...
    trk = _val(row, "tracking_code") or await _gen_tracking()
    if not _val(row, "tracking_code"):
        await execute("UPDATE orders SET tracking_code=? WHERE id=?", trk, oid)
    if row and _val(row, "tg_id"):
        await _send_with_effect(cb.bot, _val(row, "tg_id"), f"🔧 سفارش شما با کد پیگیری {trk} در حال انجام است.", "EFFECT_CREATED")
...
async def admin_order_complete(cb: CallbackQuery):
    ...
    if not _val(row, "tracking_code"):
        await execute("UPDATE orders SET tracking_code=? WHERE id=?", trk, oid)
    if row and _val(row, "tg_id"):
        await _send_with_effect(cb.bot, _val(row, "tg_id"), f"🎉 سفارش شما با کد پیگیری {trk} انجام شد.", "EFFECT_COMPLETED")
...
async def admin_order_reject(cb: CallbackQuery):
    ...
    if not _val(row, "tracking_code"):
        await execute("UPDATE orders SET tracking_code=? WHERE id=?", trk, oid)
    if row and _val(row, "tg_id"):
        await _send_with_effect(cb.bot, _val(row, "tg_id"), f"❌ سفارش شما با کد پیگیری {trk} رد شد. لطفاً با پشتیبانی در ارتباط باشید.", "EFFECT_REJECTED")
